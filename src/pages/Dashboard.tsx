...
import { fetchNotes, fetchTags, createNote, updateNote, deleteNote } from '../store/slices/noteSlice';
...
export default function Dashboard() {
  ...
  const { notes, tags: allTags, loading } = useAppSelector(state => state.notes);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-30 bg-stone-50/80 dark:bg-stone-950/80 backdrop-blur-md border-b border-stone-200 dark:border-stone-800 px-4 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight hidden sm:block">Aesthetic Notes</h1>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={toggleTheme} className="p-2 hover:bg-stone-200 dark:hover:bg-stone-800 rounded-lg transition-colors">
              {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>
            <button onClick={() => dispatch(logout())} className="p-2 hover:bg-stone-200 dark:hover:bg-stone-800 rounded-lg transition-colors text-red-500">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 sm:p-6">
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            <button 
              ...
              className={cn(
                ...
                selectedTags.length === 0 ? "bg-indigo-600 text-white shadow-md" : "bg-stone-200 dark:bg-stone-800 text-stone-600 dark:text-stone-400"
              )}
            >
              All Notes
            </button>
            {allTags.map(tag => (
              <button 
                ...
                className={cn(
                  ...
                  selectedTags.includes(tag) ? "bg-indigo-600 text-white shadow-md" : "bg-stone-200 dark:bg-stone-800 text-stone-600 dark:text-stone-400"
                )}
              >
            ...
            <div className={cn("grid gap-4", viewMode === 'grid' ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1")}>
              {pinnedNotes.map(note => (
                <NoteCard 
                  ...
                  note={note} 
                  viewMode={viewMode}
                  onClick={() => { setEditingNote(note); setIsEditorOpen(true); }}
                  onPin={() => handleUpdateNote(note._id, { is_pinned: !note.is_pinned })}
                />
              ))}
            </div>
          </section>
        ...
        <section>
          {pinnedNotes.length > 0 && <h2 className="text-xs font-semibold uppercase tracking-widest text-stone-400 mb-4">Others</h2>}
          <div className={cn("grid gap-4", viewMode === 'grid' ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1")}>
            {otherNotes.map(note => (
              <NoteCard 
                key={note._id} 
      ...
      <button onClick={handleCreateNote} className="fixed bottom-8 right-8 w-14 h-14 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full shadow-2xl flex items-center justify-center transition-transform hover:scale-110 active:scale-95 z-40">
